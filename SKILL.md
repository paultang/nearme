...
6. **Profile 质量自检：** 在 `share` 前，按以下维度评分
   - **我是谁**（10 分）：行业 + 角色是否清晰
   - **我提供**（30 分）：产品/服务具体？核心优势明确？
   - **我需求**（30 分）：需求明确且有可操作性？
   - **联系方式**（20 分）：至少有一种联系方式
   - **Tags**（10 分）：2 个以上标签

   总分 < 60 分 → 建议用户补充缺失项
   60-80 分 → 提示"可以完善 XX"，问要不要改
   > 80 分 → 直接进入下一步

   评分结果用自然语言表达，不要展示分数表格。

7. 运行 `nearme share <base-dir>/你的文件名.md`

### 用户说"看看谁在"或 list：
1. `nearme list`
2. 直接展示 CLI 的输出结果，**保留原有的（主持人）🤝 📝 标记**，不要重新排版
3. 向用户指出谁是主持人：**第 1 位（标有（主持人））就是活动组织者**。如果有新人加入，也要告诉他们"1 号是主持人"
4. 用户选编号 → 用 `nearme view <编号>` 查看单人详情
...
- **查看所有历史空间：** `nearme spaces` — 列出所有空间（含过期），标记过期状态
- **下载过期空间资料：** `nearme download --space <code>` — 过期也能下载 profiles
- 保存目录规则：`./<空间代码>/`，不混淆
...
- **禁止使用测试/演示语言：不要说"还需要测试吗"、"试试看"、"看一下效果"、"这是测试版"等。这是一个正式产品，所有交互都应该是真实使用场景**
